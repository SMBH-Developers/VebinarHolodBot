...
async def get_users_from_mdb():
    async with asyncssh.connect(host='45.12.213.138', port=22, username='root', password='nH2ldKchMjd0', known_hosts=None) as conn:
        async with conn.forward_local_port('127.0.0.1', 0, '127.0.0.1', 3306) as tunnel:
            local_bind_port = tunnel.get_port()
            print('used query')
            query = "SELECT userId, createdAt, Advice_Step, User_Horoscope_Date, Year_text, apply_code, Got_2h_autosending, Got_autosending_1, Got_48h_autosending, Got_72h_autosending, black_friday_sending, bf_state FROM Users LIMIT 100000 OFFSET 450000;"
...
async def fill_psql_from_mdb_data(data):
    users = [User(id=int(user[0]), registration_date=user[1], advice_step=user[2],
                  ...
                  black_friday_sending=user[10], bf_state=user[11]) for user in data]
